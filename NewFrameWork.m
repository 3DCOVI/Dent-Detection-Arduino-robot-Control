...
fopen(s);
pause(4);
disp('serialready') % open serial port and initiate arduino board program
fprintf(s,'1');
scan = {};
picArray={};%initialize depth picture array
picArrayG={};%initialize optical picture Array
cameraEn = 0;
while 1==1 %loop until arduino board prints 'done' then pr
  fscanf(s)
  pause(1);
  ...
  if  strcmp(scan{1,end},'trigger') % check for trigger char
      picArray{end+1}=takePic(0,2); % take depth video picture
    picArrayG{end+1}=takePic(0,1); %and normal optical camera picture with XBixkinect sensor
    scan{1,end+1} = 'space'; % prevent taking multiple pictures
  end
...
img2 = stitch2imgColored(picArrayG); %stitch colored picture together
imshow(img2) % display collored picture
%img2 = rgb2gray(img2);
img5 = sobel(img2);%edge detection
imshow(img5)
%%imshow(cutMask(img2> 55,125,900,125,500)); crop and create logical array ove brightness values over 55

function out = cutMask(img,x1,y1,x2,y2) %crop function
        img(1:x1,:) = 0;
        img(end - x2:end,:) = 0;
        img(:, 1:y1) = 0;
        img(:, end - y2 : end) = 0;
...
function scanned = takePic(timeDelay,cameraType)  
        ...
        disp('taking picture')%indicated start
        pause(timeDelay);
        start(depthVid);
        [depthFrameData,depthTimeData,depthMetaData] = getdata(depthVid);
        scanned = depthFrameData(:,:,:,framesPerTrig);
        %scanned(scanned > 1000 | scanned < 700) = 0; %depth filter
        stop(depthVid);
        % stoppreview(depthVid);
       disp('picture taken')%indicated end
       %outputs a picture
end
